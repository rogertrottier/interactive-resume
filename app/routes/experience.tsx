...
export function Experience() {
  const experiences = [
    { title: "Senior Software Engineer at Capital Benefit Group", description: "In my current role, I design and develop full-stack applications using .NET Core, Blazor, and managing the datastructure with Entity Framework. I optimize data architectures, integrate tools like Salesforce, manage deployment pipelines, and mentor new team members to ensure our solutions are scalable and efficient." },
    { title: "Full-Stack Developer at Capital Benefit Services / EPK Benefits", description: "During my nearly eight-year tenure, I specialized in developing robust applications using ASP.Net MVC and C#. My work involved generating detailed reports with Crystal Reports, modernizing legacy systems from VBA to C#, and crafting dynamic online forms with PHP, jQuery, and Bootstrap—all while delivering reliable solutions for both clients and brokers." },
    { title: "Software Developer at Prepared Response", description: "At Prepared Response, I contributed to enhancing Rapid Responder, an emergency management system critical to organizations nationwide. I developed features across the full stack—from MySQL database enhancements and web services in C# and VB to intuitive front-end interfaces using HTML, JavaScript, and jQuery—ensuring timely and actionable crisis data." },
    { title: "Database Developer at Prepared Response", description: "I led a comprehensive migration project, transitioning databases from SQL to MySQL. This involved converting schemas, procedures, views, and data, which laid the foundation for improved performance and scalability in the company's primary products." }
  ];

  // State to track the selected job
  const [selectedExperience, setSelectedExperience] = useState(experiences[0]);
  // Create a ref for the right panel container
  const rightPanelRef = useRef<HTMLDivElement>(null);

  // Animate the panel when the selected experience changes.
  useEffect(() => {
    if (rightPanelRef.current) {
      gsap.fromTo(
        rightPanelRef.current,
        { opacity: 0, x: 40 }, // starting state: offscreen right and invisible
        { opacity: 1, x: 0, duration: 0.4 } // ending state: visible and in place
      );
    }
  }, [selectedExperience]);
      ...
      <div className="flex w-full flex-col md:flex-row">
        ...
        <div className="md:w-1/3 text-white p-6 flex flex-col gap-4">
          {experiences.map((experience, index) => (
            <button
              ...
              className={`interactive text-left p-3 rounded-md transition ${selectedExperience.title === experience.title
        ...
        <div className="md:w-2/3 flex justify-center items-center p-8">
          <div ref={rightPanelRef} className="max-w-2xl">
            ...
            <p className="text-lg leading-relaxed">{selectedExperience.description}</p>
          </div>
        </div>
      </div>
    </>
